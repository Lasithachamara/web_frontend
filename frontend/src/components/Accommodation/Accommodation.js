...
function Accommodation() {
  ...
  useEffect(() => {
    fetch("http://localhost:5001/Accommodation/allAccommodation")
    ...
    <div>
      ...
      {accommodations.map((accommodation) => (
        <div className="article-card" key={accommodation._id}> {/* Add the article-card class here */}
          <img src={`http://localhost:5001/${accommodation.image}`} alt={`${accommodation.Hotel_Name}`} /> {/* Add the alt attribute for accessibility */}
