...
const ViewAccommodation = () => {
  ...
  const fetchAccommodationPackages = async () => {
    try {
      const response = await fetch('http://localhost:5001/AccommodationPackage/get-AccommodationPackage');
  ...
  useEffect(() => {
    fetch(`http://localhost:5001/Accommodation/${id}`)
  ...
  return (
    <div className="Accommodation">
      ...
      <div className="article">
        ...
        <img src={`http://localhost:5001/${accommodation.image}`} alt={`${accommodation.Hotel_Name}`} />
