...
const ManageAccommodationpackage = () => {
  ...
  useEffect(() => {
    fetch("http://localhost:5001/AccommodationPackage/allAccommodationPackage")
  ...
  function deleteAccommodationpackage(id){
    Swal.fire({
        ...
        if(res.isConfirmed)
        {axios.delete(`http://localhost:5001/AccommodationPackage/delete-AccommodationPackage/${id}`);
        Swal.fire({
            title: 'Success!',
